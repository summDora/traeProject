<template>
  <div class="hy-table newTable">
    <el-table
        v-bind="$attrs"
        v-set-id
        class="hy-table-content"
        style="width: 100%;"
        :stripe="stripe"
        ref="table"
        ...
        :border="true"
        :span-method="needSpanMethod ? spanMethod : () => ''"
    ...
    >
      ...
      <el-table-column class="radioCss" lable width="55" v-if="ifHaveRadio" :fixed="true">
        <template slot-scope="scope">
          <el-radio
              :label="scope.$index"
              v-model="radio"
              @change.native="getRadio(scope.row)"
          >{{a}}
          </el-radio>
        </template>
      </el-table-column>
      ...
      <template v-for="(item) in column">
        ...
        <template v-if="item.children">
          <newColumn v-for="item in item.children" :key="item.key" :col="item"><newColumn>
        </template>
        ...
        >
          ...
          <template slot-scope="scope">
            ...
            <div class="elTooltipCss" v-else-if="scope.row.data">
              <span
                  :class="scope.row.data[item.startPosx].posx + 'and' + scope.$index"
                  v-if="scope.row.data[item.startPosx].success"
              >{{ precisionProcessing(scope.row.data[item.startPosx], item.dataPrecision) }}</span>
              <!-- <el-tooltip  v-else placement="top" trigger="hover" :content="scope.row.data[item.startPosx].message"> -->
              <span
                  :class="scope.row.data[item.startPosx].posx + 'and' + scope.$index"
                  style="color: red;"
                  v-if="scope.row.data[item.startPosx].displayValue === undefined || scope.row.data[item.startPosx].displayValue === null || scope.row.data[item.startPosx].displayValue==='' || scope.row.data[item.startPosx].displayValue.toString().trim() === ''"
              >--</span>
              <span
                  v-else
                  :class="scope.row.data[item.startPosx].posx + 'and' + scope.$index"
        ...
        >
          <template slot="header" slot-scope="scope">
            ...
            <!-- <el-tooltip v-if="item.headerSlotZhpj == 'zhpj'" effect="dark" content='综合评价 = 业务能力 × 30% + 职业素养 × 40% + 廉洁从业 × 30%' placement="top"> -->
            <el-tooltip v-if="item.headerSlotIcon" effect="dark" :content='item.headerIconText' placement="top">
              <i class="el-icon-question"></i>
            </el-tooltip>
            <!-- <el-tooltip v-else-if="item.headerSlotZhpj == 'profesosrJx'" effect="dark" content='专家履职绩效 = 单位专家数 / 会议专家总数 × 100' placement="top">
              <i class="el-icon-question"></i>
            </el-tooltip> -->
          </template>
          <template v-if="item.children">
          ...
          <template slot-scope="scope">
            ...
            <!-- 动态配置0：文本框；1：下拉框；2：日期选择框；3：数字文本框；4：级联下拉框；5：界面编辑框；6：展示框；7：点击弹框 -->
            <div v-else-if="item.dictheadId">
              <!-- 展示框,级联 -->
                <span slot="reference" :class="scope.row.index + item.key" v-if="item.dictheadId"> {{scope.row[item.key+'Name']?(scope.row[item.key+'Name'].toString().trim() ? scope.row[item.key+'Name'] :  '--'):'--' }}</span>
           </div>
            <div v-else-if="item.typeName">
              <div v-if="item.typeName==='4'">
                <span slot="reference" :class="scope.row.index + item.key" >{{scope.row[item.key]?scope.row[item.key].toString().trim() ? scope.row[item.key+''] :  '--':'--' }}</span>
              </div>
              
              <!-- 文本框,界面编辑框-->
              <div v-if="item.typeName=='0'||item.typeName=='5'">
                <el-input v-model="scope.row[item.key]" size="mini" :disabled="allowXmzt.indexOf(scope.row.xmzt?scope.row.xmzt:'0')==-1||allowXqzt.indexOf(scope.row.xqzt?scope.row.xqzt:'0')==-1" :maxlength="item.fieldLength" @blur="inputOnBlur(scope.row)"></el-input>
              </div>
              <!-- 下拉框 -->
              <div v-if="item.typeName=='1'">
                <el-select  :clearable="true"  v-model="scope.row[item.key]" :disabled="allowXmzt.indexOf(scope.row.xmzt?scope.row.xmzt:'0')==-1||allowXqzt.indexOf(scope.row.xqzt?scope.row.xqzt:'0')==-1"  placeholder="请选择"  @change='formSelectChange(scope.row,item.key,scope.row[item.key])' collapse-tags>
                  <el-option v-for="item in Options[item.key]" :key="item.dictId" :label="item.dictName" :value="item.dictId">
                  </el-option>
                </el-select>
              </div>
               <!--日期选择框  -->
              <div v-if="item.typeName=='2'">
                <el-date-picker v-if="item.dateType=='0'||item.dateType=='1'"  :disabled="allowXmzt.indexOf(scope.row.xmzt?scope.row.xmzt:'0')==-1||allowXqzt.indexOf(scope.row.xqzt?scope.row.xqzt:'0')==-1" v-model="scope.row[item.key]" size="mini"
                 :value-format="item.dateType=='0'?'yyyy-MM':'yyyy-MM-dd'" :format="item.dateType=='0'?'yyyy-MM':'yyyy-MM-dd'" :editable="false" :clearable="true" @change="datePickerOnChange(scope.row,item.key,scope.row[item.key])"></el-date-picker>
                <el-date-picker v-else-if="item.dateType=='2'||item.dateType=='3'" type="datetime" :disabled="allowXmzt.indexOf(scope.row.xmzt?scope.row.xmzt:'0')==-1||allowXqzt.indexOf(scope.row.xqzt?scope.row.xqzt:'0')==-1" v-model="scope.row[item.key]" size="mini" 
                :value-format="item.dateType=='2'?'yyyy-MM-dd HH:mm':'yyyy-MM-dd HH:mm:ss'" :format="item.dateType=='2'?'yyyy-MM-dd HH:mm':'yyyy-MM-dd HH:mm:ss'" :editable="false" :clearable="true" @change="datePickerOnChange(scope.row,item.key,scope.row[item.key])"></el-date-picker>
                 <el-date-picker v-else :disabled="allowXmzt.indexOf(scope.row.xmzt?scope.row.xmzt:'0')==-1||allowXqzt.indexOf(scope.row.xqzt?scope.row.xqzt:'0')==-1" v-model="scope.row[item.key]" size="mini" 
                value-format="yyyy-MM-dd" format="yyyy-MM-dd" :editable="false" :clearable="true" @change="datePickerOnChange(scope.row,item.key,scope.row[item.key])"></el-date-picker>
              </div>
              <!-- 数字文本框 -->
              <div v-if="item.typeName=='3'" class="numberInput">
                <el-input v-model="scope.row[item.key]" type="number" :disabled="allowXmzt.indexOf(scope.row.xmzt?scope.row.xmzt:'0')==-1||allowXqzt.indexOf(scope.row.xqzt?scope.row.xqzt:'0')==-1"  size="small" :maxlength="item.fieldLength" @blur="inputOnBlur(scope.row)" >
                    <span 
                    slot="suffix" 
                    style="color:#409eff;font-size:15px;font-weight:700;margin-right:10px;line-height: 36px;cursor:pointer" 
                    @click="inputIconClick(item.key)"
                  >{{item.menuUnit?item.menuUnit:item.unit}}</span>
                </el-input>
                 
              </div>
              <!-- 点击弹框  -->
              <div v-if="item.typeName=='7'">
                <el-input  v-model="scope.row[item.key]" :disabled="allowXmzt.indexOf(scope.row.xmzt?scope.row.xmzt:'0')==-1||allowXqzt.indexOf(scope.row.xqzt?scope.row.xqzt:'0')==-1" @focus="openFromInput(scope.row,item.key)" readonly ></el-input>
              </div>
            </div>
            <!-- 空显示 -->
            <span
    ...
    <hyNewModal :showClose="true" v-if="editModalShow"
      :dialogVisible='editModalShow'
      :title='modalTitle' :ifSure="true" :ifCancle="true"
      @sureHandle="interfaceEdit"
      width='760px'
      @closeHandle="editModalShow=false">
        <el-input type="textarea" onkeyup='this.value=this.value.replace(/[^\u4e00-\u9fa5a-zA-Z0-9#;；（）、/,，.。:：%‘’“”!！ ]/g, "")' maxlength="200" :autosize="{minRows:3,maxRows:6}" placeholder="请输入内容"  v-model="fieldData" />
    </hyNewModal>
  </div>
</template>
...
<script>
  ...
  import {  queryDictTree } from '@/api/common'
  ...
  export default {
    ...
    watch: {
      ...
      column(){
        let commonTree=this.column.filter(item=>item.typeName=='1')
        commonTree.forEach(item=>{
          this.getCommonTree(item.menuDictheadId,item.key)
        })
      }
    },
    mounted() {
    ...
    methods: {
      ...
      cellStyle({row,column,rowIndex,columnIndex}) {
        ...
        if(this.needCellStyle){
          if(row.firstIssuesType == 'GFXWT' && (columnIndex == 0)){
            return {'background-color':'#e4effd !important'};
          }
          if(row.firstIssuesType == 'JSWT' && (columnIndex == 0)){
            return {'background-color':'#fff6eb !important'};
          }
          if(row.firstIssuesType == 'JJWT' && (columnIndex == 0)){
            return {'background-color':'#e5e8ff !important'};
          }
        }
        //如果是勾选
      ...
      getCommonTree(dictId,key){
        this.m_apiFn(queryDictTree, dictId).then(res => {
      ...
      //选择改变
      formSelectChange(row,key){
        this.$emit('formEdit',row)
      },
      // 时间改变
      datePickerOnChange(row,key){
        this.$emit('formEdit',row)
      },
      // 打开界面编辑
      openFromInput(row,key){
        this.nowRowData=this.m_copy(row) 
        this.eidtField=key
        this.fieldData=row[key]
      ...
      interfaceEdit(){
        ...
        this.$set(this.nowRowData,this.eidtField,this.fieldData)
        this.$emit('formEdit',this.nowRowData)
      },
...
<style>
  ...
  .newTable .el-table td.el-table__cell{
